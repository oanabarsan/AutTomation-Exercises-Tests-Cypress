...
const fullAddress = randomCity + " " + randomState + '\n\t\t\t\t\t\t\t\t' + randomZipCode;
...
describe("Place order and register while checkout test suite", () => {
  ...
  it("Place order and register while checkout test", () => {
    ...
    CartPage.getProceedToCheckoutBtn().click();
    cy.get('div.step-one h2.heading').contains('Address Details').should('exist');
    ...
    cy.get('div.step-one h2.heading').contains('Review Your Order').should('exist');
    cy.get('a[href="/product_details/1"]').contains("Blue Top").should("exist");
    cy.get('a[href="/product_details/2"]')
      .contains("Men Tshirt")
      .should("exist");
    ...
    cy.get('tr[id="product-2"] td.cart_total p')
      .contains(`Rs. ${priceSecondItem}`)
      .should("be.visible");
    cy.get('td[colspan="2"] h4').children('b').should('have.text', "Total Amount");
    cy.get('td p.cart_total_price').contains(`Rs. ${totalPrice}`).should('be.visible');
